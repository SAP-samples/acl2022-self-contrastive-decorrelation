...
class SCD(object):
    """
    A class for embedding sentences, calculating similarities, and retriving sentences by SCD.
    """
...
if __name__=="__main__":
    ...
    model_name = "sap-ai-research/BERT-base-uncased-SCD-ACL2022"
    scd = SCD(model_name)

    print("\n=========Calculate cosine similarities between queries and sentences============\n")
    similarities = scd.similarity(example_queries, example_sentences)
    print(similarities)

    print("\n=========Naive brute force search============\n")
    scd.build_index(example_sentences, use_faiss=False)
    results = scd.search(example_queries)
    ...
    scd.build_index(example_sentences, use_faiss=True)
    results = scd.search(example_queries)
    for i, result in enumerate(results):
        print("Retrieval results for query: {}".format(example_queries[i]))
        for sentence, score in result:
